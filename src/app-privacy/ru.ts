...
Эта Политика конфиденциальности описывает, как собирается, используется и передается ваша персональная информация, когда вы посещаете${when(conditions?.shop, ' или совершаете покупку в')} [${appName}](${website}) (далее "Приложение").

## Личная информация, которую мы собираем

Когда вы посещаете Приложение, мы автоматически собираем определенную информацию о вашем устройстве, включая информацию о вашем веб-браузере, IP-адресе, часовой зоне и некоторые из установленных на вашем устройстве файлов cookie. Кроме того, когда вы просматриваете Приложение, мы собираем информацию об отдельных веб-страницах, которые вы просматриваете, о том, с каких веб-сайтов или поисковых запросов вы перешли в Приложение, а также, как вы взаимодействуете с Приложением. Мы называем эту автоматически собираемую информацию "Информация об устройстве".

### Мы собираем информацию об устройстве${when(!conditions?.shop, ' (или Личную информацию)')} с использованием следующих технологий:

 - "Лог-файлы" отслеживают действия, происходящие в Приложении, и собирают данные, включая ваш IP-адрес, тип браузера, интернет-провайдера, страницы входа/выхода и метки даты/времени.
${when(conditions?.tracking, `
 - "Веб-маяки", "теги" и "пиксели" являются электронными файлами, используемыми для записи информации о том, как вы просматриваете Приложение.
`.trim())}

${`
${when(conditions?.shop, 'Кроме того, когда вы совершаете покупку или пытаетесь совершить покупку в Приложении, мы собираем определенную информацию о вас, включая ваше имя, адрес для выставления счета, адрес электронной почты и номер телефона. Мы называем эту информацию "Информация о заказе".')}
...
Мы используем Информацию о заказе, которую мы собираем, в основном для выполнения любых заказов, размещенных через Приложение (включая обработку вашей платежной информации, организацию доставки и предоставление вам счетов и/или подтверждений заказов). Кроме того, мы используем эту Информацию о заказе для:
 - Общения с вами;
...
`)}
Мы используем Информацию об устройстве, которую мы собираем, чтобы выявлять потенциальные риски и мошенничество (в частности, ваш IP-адрес), а также в более общем плане для улучшения и оптимизации нашего Приложения (например, путем генерации аналитических данных о том, как наши пользователи просматривают страницы и взаимодействуют с Приложением, и для оценки успеха наших маркетинговых и рекламных кампаний).

## Передача вашей личной информации

Мы делимся вашей Личной информацией с третьими сторонами, чтобы обеспечить выполнение заказанных вами обязательств. Например, мы используем хостинг-провайдера для обеспечения работы нашего Приложения.
...
`)}
## Аналитика
Обратите внимание, что мы не изменяем наши методы сбора и использования данных в Приложении, когда видим сигнал Do Not Track от вашего браузера.
...
Правовое основание ${company} для сбора и использования личной информации, описанной в этой Политике конфиденциальности, зависит от Личной информации, которую мы собираем, и конкретного контекста, в котором мы ее собираем:
 ...
 - Вы дали ${company} разрешение на этоЮ
...
Если вы являетесь резидентом Европы, мы отмечаем, что обрабатываем вашу информацию, чтобы выполнять контракты, которые у нас могут быть с вами${when(conditions?.shop, ' (например, если вы делаете заказ через Приложение)')}, или иным образом для преследования наших законных деловых интересов, перечисленных выше. Кроме того, обратите внимание, что ваша информация может быть передана за пределы Европы, включая Канаду и Соединенные Штаты.`)}
${when(conditions?.shop, `
## Хранение данных
Когда вы размещаете заказ в Приложении, мы будем хранить Информацию о заказе для наших записей, если и до тех пор, пока вы не попросите нас удалить эту информацию.
`)}
${when(minimumAge, `
## Возрастные ограничения
Приложение не предназначено для лиц моложе ${minimumAge} лет.
`)}
## Изменения
